# Shipmart — complete site brief for Stitch

Design and generate a complete multi-screen website for **Shipmart**, a
cross-border parcel shipping service for eCommerce merchants. Generate every
screen listed below as a separate screen in this project, using one consistent
...
Shipmart lets an online store ship to customers in other countries without the
usual customs risk. The merchant prints one label; Shipmart picks the lane,
files the customs paperwork, prepays the duties, and delivers to the door, so
the buyer never receives a surprise bill. The audience is small and mid-size
eCommerce merchants — handmade jewellery, eco-friendly goods, custom
merchandise — expanding into international markets for the first time.

## Voice

Lead with the merchant's outcome, never the mechanism. Not "multi-carrier rate
aggregation" but "One label. Duties paid. No surprise bill at your customer's
door." Active voice, sentence case. Controls say exactly what they do — "Get a
quote", not "Submit" — and keep the same name through a flow. Recurring words:
landed cost, cleared, tracked, delivered, no surprise duties, one label,
predictable, every border.

---

## DESIGN SYSTEM

> Delete this whole section if you have already set the project theme in
> Stitch's design settings — theme defined in two places will conflict.
...
- Midnight Harbour `#0B1327` — primary text on light surfaces, and the
  background of dark inversion sections.
- Transit Teal `#00D2C6` — secondary accent: data visualisation, route lines,
  in-motion states. **Never text on a light background** — it measures 1.90:1
  and fails. It is a fill, or text on Midnight Harbour.
- Manifest Purple `#8A2BE2` — tertiary accent: illustration gradients and chart
  series only, never a button.
- Dock Grey `#F4F6F8` — alternating section background and resting card surface.
- White `#FFFFFF` — primary surface, and text on Midnight Harbour.
- Slate Muted `#556074` — secondary text, captions, helper text.
- Hairline `#E2E8F0` — decorative dividers only, never a control border.
- Control Border `#7E8899` — input, select and checkbox borders.
- Sky Blue `#6BA3FF` — replaces Signal Blue for links and buttons on dark
  sections, where Signal Blue fails contrast at 3.21:1.
- Status: Cleared Green text `#067A4E` on fill `#10B981` · Held Amber text
  `#B45309` on fill `#F59E0B` · Exception Red text `#B91C1C` on fill `#DC2626`.

**Typography**
- Headlines: Plus Jakarta Sans, ExtraBold 800 for page headlines, Bold 700 for
  section headers. Tight line-height 1.05–1.15, slight negative letter-spacing.
- Body and labels: Inter. Regular 400 body at 1.6 line-height, Medium 500 for
  data labels, SemiBold 600 for buttons. Body never below 16px.
- Tabular numerals everywhere a figure can change — rates, weights, transit
  days, prices, tracking IDs.
- Section eyebrows uppercase, small, widely tracked. Action titles in the
  three-pillar grid are lower-case.

**Shape and depth**
- Gently rounded corners, 8px, on buttons, cards and inputs.
- Cards: white surface, hairline stroke, no shadow or a whisper-soft diffused
  one. Depth comes from strokes and background alternation, not stacked shadows.
- Status badges are pill-shaped, tinted fill with matching dark text, and always
  carry a word — status is never conveyed by color alone.
- SVG line icons at consistent stroke weight. Never emoji as icons.

**Layout**
- Strict 12-column grid, 8px base unit, spacing scale 8/16/24/32/48/64/96/128.
- Section vertical padding 96–128px desktop, 64px mobile. The site breathes —
  one idea absorbed per section.
- Max content width ~1200px; prose capped near 65 characters.
- Asymmetric by default: headline left, visual right, alternating down the page.
  Centring is reserved for the closing call to action.
- Breakpoints 375 / 768 / 1024 / 1440. Tables scroll inside their own container
  rather than breaking the layout. Nothing important is hover-only.
- Motion 150–300ms ease-out, used for state changes and a light staggered fade
  of sections on first scroll. Respect reduced-motion.

**Atmosphere**
Engineered clarity — the calm of a well-run port, not the noise of a courier
depot. The signature move is **interface over photography**: where a competitor
shows a warehouse photo, Shipmart shows the product surface — a landed-cost
breakdown, a customs timeline, a lane comparison, a tracking card. No stock
photography of boxes, planes or warehouses anywhere on the site.

---

## SHARED COMPONENTS

Used on the screens that reference them below.
...
1. **Header**: the standard site header defined above.
...
10. **Footer**: the standard site footer defined above.
...
8. **Footer**: the standard site footer defined above.


---

### Screen 3 — Pricing & rates
...
9. **Footer**: the standard site footer defined above.
...
### Screen 5 — Destinations & coverage
...
7. **Footer**: the standard site footer defined above.
...
### Screen 15 — Prohibited & restricted items
...
Every `[[double-bracketed]]` item is an unverified fact — a count, a statistic,
a price, a partner name. Replace each with a real value or delete the element
around it. Do not render them as written; a fabricated statistic that looks
real is worse than a missing one. The highest-risk are the three figures in the
Home problem section and the numbers band on the About screen.

## QUALITY BAR

Text contrast at least 4.5:1 in every combination. Visible focus states on all
interactive elements. Hit targets at least 44x44px with 8px separation. Every
form input has a permanently visible label — never placeholder-only. Validation
on blur, errors beneath the offending field in words, explicit loading and
success states on submit. Responsive at 375, 768, 1024 and 1440 with no
