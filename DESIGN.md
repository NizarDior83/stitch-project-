...
theme:
  ...
  overridePrimaryColor: "#0052FF"
  overrideSecondaryColor: "#00D2C6"
  overrideTertiaryColor: "#8A2BE2"
  overrideNeutralColor: "#0B1327"
  spacingScale: 3
---

# Shipmart — Stitch Design Specification

**Project ID:** `[[fill in after create_project]]`
**Product:** Cross-border parcel shipping for eCommerce merchants.
**Device target:** Web, desktop-first. Mobile variants called out per page.

---

## How to use this file

Stitch splits a design spec into two layers, and mixing them **breaks generation**.

| Layer | What it holds | Where it goes |
|---|---|---|
| **Part A** (frontmatter + §1–§6) | Colors, fonts, roundness, spacing, component styling, voice | Uploaded once as `DESIGN.md` → becomes the project design system |
| **Part B** (§B0–§B15) | Page structure, sections, content, copy | Pasted per screen into `generate_screen_from_text` |

**The rule that matters:** once Part A is uploaded, page prompts must contain **zero** hex codes, font names, color words, or theme instructions. Stitch applies those at project level, and repeating them in a prompt makes the two fight. Part B below is already written clean — keep it that way when you edit.

**Upload sequence:**
1. `create_project` → capture the project ID, write it in above.
2. Upload this file via `upload_to_stitch.py --file-path DESIGN.md` (the script base64-encodes in-process, avoiding token limits). Returns a `sourceScreen` ID.
3. `get_project` → find the matching `screenInstance` ID.
4. `create_design_system_from_design_md` with `{projectId, selectedScreenInstance:{id, sourceScreen}, deviceType:"DESKTOP"}`. This parses the YAML frontmatter above and populates every theme token — no `update_design_system` call needed after it.
5. Generate each screen from Part B, passing the returned `designSystem` ID.

**Two conventions used below:**
- `[[double brackets]]` = an unverified fact — a count, a stat, a price, a partner name. **Every one must be confirmed or deleted before this goes to Stitch**, or the design bakes in a fabricated claim.
- §B0 holds the global header and footer. Each page prompt references them; paste the §B0 text into the prompt in place of the reference when you send it.
...
Shipmart should feel like **infrastructure you can see through** — the calm of a well-run port, not the noise of a courier depot. The governing mood is *engineered clarity*: confident, spacious, quietly technical, with nothing decorative competing for attention.

The site breathes. Sections are separated by generous vertical air so a merchant absorbs exactly one idea before meeting the next. Backgrounds alternate between white and a cool off-white so the rhythm of the page is legible without rules or boxes doing the work.

The signature move is **interface-over-photography**. Where a competitor would show a photograph of a warehouse or a smiling courier, Shipmart shows the actual product surface: a landed-cost breakdown, a customs status timeline, a lane comparison, a tracking card. This is the single strongest trust signal available — it tells a merchant the software exists and is precise, before they read a word of marketing. Stock photography of boxes, planes, or warehouses is off-brand and should not appear anywhere.

Density is low in marketing pages and high in utility pages. A pricing table or a tracking timeline is allowed to be dense and data-rich; a hero is not.

## 2. Color Palette & Roles

Every pairing below was contrast-tested. The ratio and its WCAG verdict are stated because several of these colors are **safe as fills and unsafe as text**, and that distinction is the most common way this palette gets misused.

### Core brand

| Name | Hex | Role |
|---|---|---|
| **Signal Blue** | `#0052FF` | The single action color. Primary buttons, links, active states, focus rings, selected lane. Never used as a large decorative wash. |
| **Midnight Harbour** | `#0B1327` | Primary text on light surfaces, and the background of dark inversion sections (problem section, closing block, footer). |
| **Transit Teal** | `#00D2C6` | Secondary accent. Data visualisation, route lines, "in motion" states, illustration highlights. |
| **Manifest Purple** | `#8A2BE2` | Tertiary accent. Illustration gradients and chart series only — never a button. |
| **Dock Grey** | `#F4F6F8` | Alternating section background and resting card surface. |
| **White** | `#FFFFFF` | Primary surface, card surface on grey sections, text on Midnight Harbour. |

### Supporting — added to close real gaps

The six brand colors carry no muted text tone, no border tone, and no status system. A parcel product cannot function without status. These are additions:

| Name | Hex | Role |
|---|---|---|
| **Slate Muted** | `#556074` | Secondary body text, captions, table meta, form helper text. |
| **Hairline** | `#E2E8F0` | Decorative dividers and section separators **only**. |
| **Control Border** | `#7E8899` | Input, select, and checkbox borders — anything a user must perceive as an interactive boundary. |
| **Cleared Green** | `#067A4E` text / `#10B981` fill | Delivered, customs cleared, payment succeeded. |
| **Held Amber** | `#B45309` text / `#F59E0B` fill | Held at customs, action needed, address unconfirmed. |
| **Exception Red** | `#B91C1C` text / `#DC2626` fill | Failed delivery, returned to sender, rejected. |
| **Sky Blue** | `#6BA3FF` | Substitute for Signal Blue on Midnight Harbour backgrounds. |

### Verified contrast

| Pairing | Ratio | Verdict |
|---|---|---|
| Midnight Harbour on White | 18.47:1 | Pass AAA |
| Midnight Harbour on Dock Grey | 17.05:1 | Pass AAA |
| White on Midnight Harbour | 18.47:1 | Pass AAA |
| Signal Blue text on White | 5.75:1 | Pass AA |
| Signal Blue text on Dock Grey | 5.31:1 | Pass AA |
| White label on Signal Blue button | 5.75:1 | Pass AA |
| Manifest Purple text on White | 5.96:1 | Pass AA |
| Slate Muted on White / Dock Grey | 6.34:1 / 5.85:1 | Pass AA |
| Midnight Harbour on Transit Teal fill | 9.71:1 | Pass AAA |
| Sky Blue on Midnight Harbour | 7.31:1 | Pass AA |
| Transit Teal on Midnight Harbour | 9.71:1 | Pass AAA |
| Control Border on White | 3.58:1 | Pass (non-text 3:1) |

### Three hard rules that follow from the measurements

1. **Transit Teal is never text on a light background.** It measures 1.90:1 on white — roughly four times below the floor. It is a fill, a stroke, a chart series, or text on Midnight Harbour (where it is excellent at 9.71:1).
2. **Signal Blue is never a link or button on Midnight Harbour.** It measures 3.21:1 against navy and fails. Dark sections use Sky Blue `#6BA3FF` for the same job.
3. **Hairline is never an input border.** At 1.23:1 it is invisible as a control boundary. Decorative separation only; interactive boundaries use Control Border.

## 3. Typography Rules

Two families, both from Stitch's supported set.

- **Headlines — Plus Jakarta Sans.** Geometric, slightly humanist, modern without novelty. Used at ExtraBold 800 for page-level headlines and Bold 700 for section headers. Headlines run tight: line-height 1.05–1.15 at display sizes, with negative letter-spacing (roughly −0.02em) so large type reads as one deliberate mass rather than spaced-out words.
- **Body and labels — Inter.** Regular 400 for paragraphs, Medium 500 for table and data labels, SemiBold 600 for button labels. Body line-height is 1.6 for comfort, dropping to 1.45 for lede paragraphs. Body never goes below 16px; table meta never below 14px.

Numerals are tabular everywhere a figure can change — rates, weights, dimensions, transit days, tracking IDs, prices. Misaligned digits in a shipping table read as sloppiness in a business whose entire promise is precision.

Section eyebrows are uppercase Inter Medium at small size with wide tracking (~0.14em). Action titles in the three-pillar grid are **lower-case** — a deliberate signal borrowed from the reference direction that keeps functional labels feeling like software rather than advertising.

## 4. Component Stylings

- **Buttons.** Gently rounded corners (8px). Primary: solid Signal Blue, white SemiBold label, generous horizontal padding, no gradient. Secondary: white surface, Control Border stroke, Midnight Harbour label. Tertiary: text-only in Signal Blue with underline on hover. All transitions 200ms. Every button shows a visible focus ring in Signal Blue offset from the edge. Minimum hit target 44×44px.
- **Cards and containers.** Gently rounded (8px), white surface, Hairline stroke, and either no shadow or a whisper-soft diffused shadow. Depth in this system comes from **the stroke and the background alternation, not from shadow stacking**. Cards never nest more than one level.
- **Inputs and forms.** White surface, Control Border stroke, 8px corners, comfortable 44px+ height. Every input carries a permanently visible label above it — placeholder-only labelling is prohibited, since the label vanishes exactly when the user needs it. Helper text sits below in Slate Muted. Validation runs on blur, not on submit. Errors appear directly beneath the offending field in Exception Red with an icon, never collected in a summary at the top. Submitting always produces a visible loading state followed by an explicit success or failure message.
- **Status badges.** Pill-shaped, small, tinted fill with the matching dark text tone. Status is **never conveyed by color alone** — every badge carries a word, and where relevant an icon, so it survives colorblindness and greyscale printing.
- **Tables.** Hairline row separators, no vertical rules, no zebra striping. Header row in Inter Medium, Slate Muted, uppercase, small. On narrow screens tables either scroll horizontally inside their own container or collapse into stacked cards — the page body itself never scrolls sideways.
- **Data widgets.** The recurring hero and section anchors. Crisp vector interface panels — cost breakdowns, timelines, lane comparisons, tracking cards. Rendered flat with Hairline strokes, Transit Teal and Manifest Purple for series, Signal Blue for the active element.
- **Icons.** SVG line icons at consistent stroke weight (Lucide or Heroicons). **Emoji are never used as icons.**

## 5. Layout Principles

A strict 12-column grid with mathematical spacing. Base unit 8px; the spacing scale runs 8 / 16 / 24 / 32 / 48 / 64 / 96 / 128. Section vertical padding is large — 96px to 128px on desktop, 64px on mobile — because whitespace is doing the pacing work.

Content sits in a max-width container of roughly 1200px, with prose blocks capped near 65 characters so paragraphs stay readable regardless of viewport. Full-bleed is reserved for dark inversion sections and the closing block.

Layouts are asymmetric by default: headline left, supporting visual right, alternating direction down the page. Perfect centering is reserved for the closing call-to-action, where symmetry signals finality.

Breakpoints: 375 / 768 / 1024 / 1440. Mobile-first behavior — single column, stacked cards, header collapsing to a hamburger with the primary CTA remaining visible outside the menu. Nothing important is hover-dependent, since touch devices have no hover.

Motion is restrained: 150–300ms, ease-out, used for state changes and a light staggered fade-in of sections on first scroll. Motion conveys meaning or it does not ship. `prefers-reduced-motion` is respected throughout.

## 6. Voice & Content Rules

The narrative arc across the site is **Benefit → Pain → Network**: lead with what the merchant gains, name the specific thing that hurts today, then show that Shipmart is a growing network rather than a single vendor.

Lead with the merchant's outcome, never the mechanism. Not "multi-carrier rate aggregation with automated HS classification" but "One label. Duties paid. No surprise bill at your customer's door." The jargon belongs on the integrations and help pages, where a reader has already opted in.

Recurring power words: **landed cost, cleared, tracked, delivered, no surprise duties, one label, predictable, every border.**

Write in active voice and sentence case. A control says exactly what it does — "Get a quote," not "Submit" — and keeps that name through the whole flow, so the button that says "Create shipment" produces a confirmation that says "Shipment created." Errors explain what went wrong and how to fix it, in the interface's voice, without apologising. Empty states invite an action rather than announcing emptiness.

Never state a number, country count, delivery time, price, or partner name that has not been verified. Placeholders are marked `[[like this]]` throughout Part B.

---

# PART B — PAGE PROMPTS

15 pages. Each block below is a complete Stitch generation prompt. Paste one per `generate_screen_from_text` call. **They deliberately contain no colors, fonts, or hex codes.**

## Site map

| # | Page | Group | deviceType | Priority |
|---|---|---|---|---|
| B1 | Home | Marketing | DESKTOP (+MOBILE) | Core |
| B2 | How it works | Marketing | DESKTOP | Core |
| B3 | Pricing & rates | Marketing | DESKTOP | Core |
| B4 | Integrations | Marketing | DESKTOP | Core |
| B5 | Destinations & coverage | Marketing | DESKTOP | Core |
| B6 | About | Marketing | DESKTOP | Secondary |
| B7 | Track a parcel | Utility | DESKTOP + MOBILE | Core |
| B8 | Get a quote | Utility | DESKTOP + MOBILE | Core |
| B9 | Help centre | Utility | DESKTOP | Secondary |
| B10 | Contact & support | Utility | DESKTOP | Secondary |
| B11 | Sign in | Account | DESKTOP | Core |
| B12 | Create account | Account | DESKTOP | Core |
| B13 | Terms of service | Legal | DESKTOP | Required |
| B14 | Privacy policy | Legal | DESKTOP | Required |
| B15 | Prohibited & restricted items | Legal | DESKTOP | Required |

## B0 — Global header and footer

Paste these two blocks into every page prompt in place of the `[GLOBAL HEADER]` and `[GLOBAL FOOTER]` references.

**[GLOBAL HEADER]**
> Sticky navigation bar, contained width. Left: "Shipmart" wordmark. Centre: four text links — "How it works", "Pricing", "Integrations", "Coverage". Right: a text link "Track a parcel", a secondary button "Sign in", and a primary button "Get a quote". On screens under 1024px the centre links collapse into a hamburger menu while the primary button stays visible in the bar.

**[GLOBAL FOOTER]**
> Four-column footer on a dark full-bleed background, with a fifth stacked block for the wordmark and a one-line description. Columns: **Product** (How it works, Pricing, Integrations, Coverage, Track a parcel) · **Support** (Help centre, Contact, Prohibited items, Claims) · **Company** (About, Careers, Press) · **Legal** (Terms of service, Privacy policy, Cookie settings). Below the columns, a divider, then a row with the copyright line on the left and lower-case social text-links — x, linkedin, instagram — on the right. No social icons, text only.
...
1. **Header**: [GLOBAL HEADER], reduced — wordmark and a single "Get a quote" button only.
...
9. **Footer**: [GLOBAL FOOTER], condensed to legal links and social text-links only.
...
1. **Header**: [GLOBAL HEADER], reduced.
...
10. **Footer**: [GLOBAL FOOTER], condensed.
...
# PART C — OPERATING NOTES

## Pre-flight checklist

Before generating, confirm:

- [ ] Every `[[placeholder]]` is replaced with a verified fact or the surrounding element is cut. The home page problem section (B1 §7) and the About numbers band (B6 §4) are the highest-risk — fabricated statistics presented as fact are the single worst failure mode in this file.
- [ ] Integration names in B4 correspond to integrations that actually exist.
- [ ] Legal copy in B13 and B14 has been drafted or reviewed by a lawyer.
- [ ] The design system has been created from this file's frontmatter and returns a `designSystem` ID.
- [ ] No page prompt contains a hex code, a font name, or a color word.

After generating, verify against the delivery checklist:

- [ ] Text contrast at least 4.5:1 in every light-mode combination used.
- [ ] Focus states visible for keyboard navigation on every interactive element.
- [ ] Hit targets at least 44×44px with at least 8px separation.
- [ ] Hover states with 150–300ms transitions; nothing important is hover-only.
- [ ] SVG icons throughout — no emoji used as icons.
- [ ] Responsive at 375, 768, 1024, and 1440px with no horizontal body scroll.
...
You gave the name, the palette, and the purpose. Everything below was my call — change any of it.

| Decision | Choice | Reasoning |
|---|---|---|
| Page list | 15 pages across marketing, utility, account, legal | You asked for all pages without naming them. Track-a-parcel and prohibited-items are non-negotiable for a parcel service and are usually forgotten. |
| Headline font | Plus Jakarta Sans | Stitch accepts only nine fonts. The reference direction pointed at a geometric neo-grotesk; this is the closest available with real character. |
| Body font | Inter | Neutral, excellent at small sizes and in dense tables, tabular numerals. |
| Roundness | `ROUND_EIGHT` | Gently rounded reads as modern software; `ROUND_TWELVE` reads consumer-friendly, which undercuts a logistics trust posture. |
| Color mode | `LIGHT` | Your palette is a light system — navy text, white and grey surfaces. Dark is used as inversion bands, not as the base mode. |
| Color variant | `FIDELITY` | Keeps `#0052FF` exact rather than letting Stitch's dynamic color system drift the brand hue. |
| Secondary vs tertiary | Teal secondary, purple tertiary | You described teal as a UI highlight and purple as illustration/gradient. That maps to Material's secondary and tertiary roles respectively. |
| Colors added | Slate Muted, Hairline, Control Border, three status pairs, Sky Blue | Your six had no muted text, no border tone, and no status system. A parcel product cannot express "held at customs" without one. |
| Voice | Benefit → Pain → Network, merchant-outcome first | Taken from the reference site's arc, applied to customs and duties rather than energy. |
| Device target | Desktop-first, with mobile variants for Home, Track, Quote | Merchants quote on desktop; recipients track on phones. |

## Known gaps

- **Frontmatter key names are unverified.** The Stitch documentation states that `create_design_system_from_design_md` parses theme tokens from the uploaded file's YAML frontmatter, but does not publish the exact key names. The keys above mirror the `designSystem.theme` object from the tool schema, which is the most likely match. If the parse comes back with defaults instead of these values, the same values can be pushed directly via `update_design_system` — the schema for that call is documented and confirmed.
- **The reference site was never fetched.** This environment's network policy blocked `godaylight.com`, so the structural DNA came from your written extraction rather than my own inspection. Its hedged values — "#e2e8f0 equivalent", "similar to Inter, SF Pro, or Univers" — were treated as directional only, never copied as tokens.
- **No page has been generated.** This file is the specification only, as requested.
